<template>
  ...
  const uploadImages = async (spotBoardId) => {
    ...
    try {
      ...
      const url = `http://localhost/api/board/spots/${spotBoardId}/uploadImage`;
      console.log('url: ',url);
  ...
  const submitForm = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost/api/board/spots/",
        form.value,
        {
          headers: {
            Authorization: `${userStore.jwtToken.accessToken}`,
      ...
      const spotBoardId = response.data.data;
      console.log("spotBoardId: ",spotBoardId);
      console.log("image: ",imageFiles)
  
  
      // uploadImages();
      // await axios.post(
      //   `http://localhost/api/board/spots/${spotBoardId}/uploadImage`,
      //   formData,
      //   {
      //     headers: {
      //       Authorization: `${userStore.jwtToken.accessToken}`,
      ...
      //   }
      // );
      const uploadedImageUrls = await uploadImages(spotBoardId);
      alert("게시글이 작성되었습니다.");
