...
const fetchSpots = async (limit = 12, offset = 0) => {
  try {
    ...
    // const spotImgUrl = 'file://'+spot.imgUrl;
    if (response.data.data && Array.isArray(response.data.data)) {
      cards.value = response.data.data.map( spot => ({
        id: spot.spotBoard.id,
        imageUrl: 'src/assets/images/spot_board/'+spot.spotBoard.id+'.jpeg' || defaultImage,
      ...
      <v-hover v-slot="{ isHovering, props }">
        ...
        >
          ...
          <v-overlay
            ...
            contained
          >
            <router-link :to="{ name: 'spotBoardDetail', params: { id: card.id } }">
